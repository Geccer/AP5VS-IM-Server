		...
		public void run() {
			try {
				...
				while ((request = reader.readLine()) != null) { // pøila od mého klienta nìjaká zpráva?
					...
					if (username == null) {
						username = request.trim();
						System.out.println("DBG>Client " + clientID + " set username to " + username);
						continue;
					}
					if (request.startsWith("#")) {
						...
						if (request.startsWith("#join")) {
							String g = request.substring(6).trim();
							if (!groups.contains(g)) groups.add(g);
							continue;
						}
						if (request.startsWith("#leave")) {
							String g = request.substring(7).trim();
							groups.remove(g);
							continue;
						}
						if (request.startsWith("#groups")) {
							messages.offer("Your groups: " + String.join(", ", groups));
							continue;
						}
					}
